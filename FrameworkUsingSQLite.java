//Updated Framework program using SQLite database.
import java.sql.*;
...
class Framework
{
	...
	private void getMenu() throws SQLException
	{
		...
		resultSet.next();
		String tempMenu = resultSet.getString("Content");
		menu = tempMenu.split(", ");
	}
	...
	public void insertRecord() throws SQLException 
	{
		...
		fieldValues += "'A'";
		String query = "INSERT INTO My_Table1 Values(" + fieldValues + ")";
	...
	{
		String query = "SELECT * FROM My_Table1 WHERE Status = 'A'";
		resultSet = statement.executeQuery(query);
		int numOfRecords = 0;
	...
	public void searchRecord() throws SQLException
	{
		...
		{
			String query = "SELECT * FROM My_Table1 WHERE Status = 'A' and " + fieldNames[0] + " = " + idToSearchRecord;
			resultSet = statement.executeQuery(query);
			if(resultSet.next())
			{
				printRecord(resultSet);
	...
	public void updateRecord() throws SQLException
	{
		...
		if(checkRecordPresentOrNot(idToUpdateRecord) == true)
		{
			...
			String updateQuery = "UPDATE My_Table1 SET "+ updateFieldName + " = " + newFieldValue + " WHERE " + fieldNames[0] + " = " + idToUpdateRecord;
	...
	{
		String query = "SELECT * from My_Table1 WHERE Status = 'A' and " + fieldNames[0] + " = " + fieldValue;
		resultSet = statement.executeQuery(query);
		if(resultSet.next())
		{
			return true;
	...
	public void deleteRecord() throws SQLException
	{
		...
		{
			String query = "UPDATE My_Table1 SET Status = 'D' where " + fieldNames[0] + " = " + idToDeleteRecord;
